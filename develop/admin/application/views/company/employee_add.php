				...
				<div class="row">
					<div class="form-group col-sm-4 has-feedback">
						<input type="text" class="form-control" id="admin_name"
							required="required" placeholder="管理员名字" />
					</div>
					<div class="form-group col-sm-4 has-feedback">
						<input type="text" class="form-control" id="admin_pwd"
							required="required" placeholder="管理员登录密码" />
					</div>
				</div>
			</div>
			<div class="col-md-12">
				<div class="form-group form_btn_line">
					<div class="col-sm-3 col-sm-offset-0 text-center">
						<button class="btn btn-primary btn-block" id="add" value="0">添加管理员</button>
					</div>
				</div>
				<br>
				<br>
...
$("#add").on('click',function(){
	var admin_info = {};
	admin_info.admin_name = $("#admin_name").val();
	admin_info.admin_pwd   = $("#admin_pwd").val();
	post("<?php echo base_url('api/employee_add');?>",admin_info,function(data){
			console.log(data);
